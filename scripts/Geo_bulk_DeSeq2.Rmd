...
GSE134869 <- read.table("~/t.gondi_dc/data/cnt/GSE134869_SP7_umi_counts.tsv.gz", sep = "\t", row.names = 1, header = T)
...
generate_DE_results(dds = dds, c("Zymosan", "Ctrl"), log2cutoff = 2)
generate_DE_results(dds = dds, c("HMW", "Ctrl"), log2cutoff = 2)
generate_DE_results(dds = dds, c("SeV", "Ctrl"), log2cutoff = 2)
generate_DE_results(dds = dds, c("cGAMP", "Ctrl"), log2cutoff = 2)
generate_DE_results(dds = dds, c("CpGB", "Ctrl"), log2cutoff = 2)
...
nrow(res_PAM)
head(res_PAM)
res_HMW <- read.csv("~/t.gondi_dc/scripts/HMW_vs_Ctrl_padj_cutoff.csv", header = T)
res_SeV <- read.csv("~/t.gondi_dc/scripts/SeV_vs_Ctrl_padj_cutoff.csv", header = T)
res_cGAMP <- read.csv("~/t.gondi_dc/scripts/cGAMP_vs_Ctrl_padj_cutoff.csv", header = T)
res_cpGB <- read.csv("~/t.gondi_dc/scripts/CpGB_vs_Ctrl_padj_cutoff.csv", header = T)
res_Zymosan <- read.csv("~/t.gondi_dc/scripts/Zymosan_vs_Ctrl_padj_cutoff.csv")
...
res_PAM <- subset(res_PAM, res_PAM$log2FoldChange > 0)
res_HMW <- subset(res_HMW, res_HMW$log2FoldChange > 0)
res_cGAMP <- subset(res_cGAMP, res_cGAMP$log2FoldChange > 0)
res_cpGB <- subset(res_cpGB, res_cGAMP$log2FoldChange > 0)
res_SeV <- subset(res_SeV, res_SeV$log2FoldChange > 0)
res_Zymosan <- subset(res_Zymosan, res_Zymosan$log2FoldChange > 0)

#get the vector with the gene names only
genelist_LPS <- res_LPS$gene
genelist_PAM <- res_PAM$gene
genelist_HMW <- res_HMW$gene
genelist_cGAMP <- res_cGAMP$gene
genelist_cpGB <- res_cpGB$gene
genelist_SeV <- res_SeV$gene
genelist_Zymosan <- res_Zymosan$gene
...
write.table(genelist_HMW, "~/t.gondi_dc/res/GEO_comparison/HMW_pos_log2FC.tsv", sep = "\t", quote = F)
write.table(genelist_cGAMP, "~/t.gondi_dc/res/GEO_comparison/cGAMP_pos_log2FC.tsv", sep = "\t", quote = F)
write.table(genelist_cpGB, "~/t.gondi_dc/res/GEO_comparison/cpGB_pos_log2FC.tsv", sep = "\t", quote = F)
write.table(genelist_SeV, "~/t.gondi_dc/res/GEO_comparison/SeV_pos_log2FC.tsv", sep = "\t", quote = F)
write.table(genelist_Zymosan, "~/t.gondi_dc/res/GEO_comparison/Zymosan_pos_log2FC.tsv", sep = "\t", quote = F)
```
