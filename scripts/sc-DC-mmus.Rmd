...
dc.12h.markers <- subset(dc.12h.markers, p_val_adj < 0.01 & dc.12h.markers$avg_logFC > 2)
...
#make a table for these genes to export them 
...
```{r, look at correlation between genes in cluster 3 and cluster 5}

cl2.genes <- subset(se.dc.markers, se.dc.markers$cluster == 2) 
cl5.genes <- subset(se.dc.markers, se.dc.markers$cluster == 5)

cor.12h <- cor(t(as.matrix(se.dc.mus@assays$SCT@data[c(dc.markers.LDM.12h$gene, dc.markers.PTG.12h$gene), ])))

corrplot(cor.12h , method="color", tl.col = "black", order = "FPC", col=colorRampPalette(c("blue4","white","red3"))(200), cl.lim = c(-1,1))

```


```{r, display the differential expressed genes in a heatmap, width = 20, height =10}
se.dc.mus <- SetIdent(se.dc.mus, value = "cell_class")
...
dc.markers.ctrl <- subset(dc.markers.original, dc.markers.original$cluster == "ctrl")
dc.markers.LDM.12h <- subset(dc.markers.original, dc.markers.original$cluster == "LDM_12h")
dc.markers.LDM.3h <- subset(dc.markers.original, dc.markers.original$cluster == "LDM_3h")
dc.markers.LDM_Lys3h <- subset(dc.markers.original, dc.markers.original$cluster == "LDM_Lys3h")
dc.markers.LPS <- subset(dc.markers.original, dc.markers.original$cluster == "LPS")
dc.markers.PTG.12h <- subset(dc.markers.original, dc.markers.original$cluster == "PTG_12h")
dc.markers.PTG.3h <- subset(dc.markers.original, dc.markers.original$cluster == "PTG_3h")
dc.markers.PTG_Lys3h <- subset(dc.markers.original, dc.markers.original$cluster == "PTG_Lys3h")
...
```
I would like to compare the differential expressed genes for LDM and PTG at 12h between mature and immature cells in more detail and display them in a nicer fashion than a volcano-plot, therefore I will subset the data to only have mature and immature PTG and LDM unfected cells at 12 hours
...
```{r, attach the GO terms to the tables}
