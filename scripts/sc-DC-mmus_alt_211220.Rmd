...
#get cells that are in the new data frame and not in the old one
diff_cells <- setdiff(colnames(data_new), colnames(data_old))

#193 cells were filtered 

##Look at the cells in the seurat object 
...
se.dc.mus <- CreateSeuratObject(nmat, min.cells = 3, min.features = 1000, project = "DC-TG-FH-M", meta.data = meta_new)
...
```{r, add metadatacolumn to compare with the old data}

se.dc.mus$qc_thr <- ifelse(colnames(se.dc.mus) %in% diff_cells, paste("failed"), paste("passed"))
...
se.dc.mus <- subset(se.dc.mus, subset = percent.mt < 1)
se.dc.mus <- subset(se.dc.mus, subset = nCount_RNA > 100000)
se.dc.mus <- se.dc.mus[-grep(pattern = "mt-", x = rownames(se.dc.mus)), ]


#Number of UMIs per sample
se.dc.mus@meta.data %>% 
  	ggplot(aes(color=qc_thr, x=nCount_RNA, fill= qc_thr)) + 
...
se.dc.mus@meta.data %>% 
  	ggplot(aes(x=qc_thr, fill=qc_thr)) + 
  	geom_bar() +
  	theme_classic() +
...
se.dc.mus@meta.data %>% 
  	ggplot(aes(color=qc_thr, x=nFeature_RNA, fill= qc_thr)) + 
  	geom_density(alpha = 0.2) + 
  	theme_classic() +
  	scale_x_log10() + 
...
se.dc.mus@meta.data %>% 
  	ggplot(aes(x=qc_thr, y=log10(nFeature_RNA), fill=qc_thr)) + 
  	geom_boxplot() + 
  	theme_classic() +
  	theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  	theme(plot.title = element_text(hjust=0.5, face="bold")) +
...
#Regress out the plate origin
se.dc.mus <- SCTransform(se.dc.mus, vars.to.regress = c("nCount_RNA", "orig.ident"))
...
FeaturePlot(se.dc.mus, reduction = "pca", features = "percent.mt")
DimHeatmap(se.dc.mus, dims = 1:20, cells = 500, balanced = TRUE)

ElbowPlot(se.dc.mus, ndims= 20, reduction ="pca")

se.dc.mus <- FindNeighbors(se.dc.mus, dims = 1:16)
se.dc.mus <- FindClusters(se.dc.mus) #start with default resolution of 0.8
#inspect the clusters
head(Idents(se.dc.mus), 5)

se.dc.mus <- RunUMAP(se.dc.mus, dims = 1:16, reduction = "pca")
...
DimPlot(se.dc.mus, reduction = "umap", group.by = "qc_thr")

FeaturePlot(se.dc.mus, features = "percent.mt")
FeaturePlot(se.dc.mus, features = "nCount_RNA")
FeaturePlot(se.dc.mus, features = "nCount_RNA")
...
markers_4 <- FindMarkers(se.dc.mus, ident.1 = 3)
