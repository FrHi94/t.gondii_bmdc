...
genes.table.tg <- read.table("~/t.gondi_dc/data/meta/genes.table_tg.txt",sep = "\t",header = T)
genes.table.tg$X <- NULL
genes.table.tg$Input.ID<- NULL
...
which(genes.table.tg[,1] == "TGME49_227280")
...
#rename the clusters
new.cluster.ids <- c("Tg 1", "Tg 2", "Tg 3", "Tg 4")
names(new.cluster.ids) <- levels(se.toxo)
se.toxo <- RenameIdents(se.toxo, new.cluster.ids)
levels(se.toxo) <- as.factor(c("Tg 1", "Tg 2", "Tg 3", "Tg 4"))
levels(se.toxo)
       

#alternatively we can rename the seurat clusters colomun right away:
se.toxo$seurat_clusters <- ifelse(se.toxo$seurat_clusters == "0", paste("Tg 3"), ifelse(se.toxo$seurat_clusters == "1", paste("Tg 1"), ifelse(se.toxo$seurat_clusters == "2", paste("Tg 4"),paste("Tg 2"))))

DimPlot(se.toxo, group.by = "seurat_clusters", cols = toxo.c)
...
p2 <- DimPlot(se.toxo, group.by = "seurat_clusters",cols = c("#F6511D", "#FFB400", "#00A6ED", "#DA70D6"))+
                              theme(axis.ticks = element_blank(),
                              axis.text = element_blank(), 
                              axis.title = element_blank())
...
DimPlot(se.toxo, group.by = "Phase",cols = c("darkolivegreen", "royalblue4", "darkorange3")) +  
      theme(axis.ticks = element_blank(),
      axis.text = element_blank(), 
      axis.title = element_blank())
