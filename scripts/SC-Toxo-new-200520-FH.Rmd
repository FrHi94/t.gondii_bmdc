...
DimPlot(se.toxo, group.by = "cell_class", cols = c("darkgreen", "darkolivegreen2", "darkblue", "cornflowerblue"))
...
umap.tx.cellclass <- DimPlot(se.toxo, reduction = "umap", 
                               group.by = "cell_class", 
                               cols = c("forestgreen", "olivedrab3", "blue3","dodgerblue3"))  +
                              theme(axis.ticks = element_blank(),
                              axis.text = element_blank(), 
                              axis.title = element_blank())

umap.tx.cellcycle <- DimPlot(se.toxo, group.by = "Phase",cols = c("darkolivegreen", "royalblue4", "darkorange3")) +  
                              theme(axis.ticks = element_blank(),
                              axis.text = element_blank(), 
                              axis.title = element_blank())

png(filename = "~/t.gondi_dc/res/tx_umap.png", width = 50, height = 15, res = 600, units = "cm") #Units are pixels by default
print(cowplot::plot_grid(umap.tx.cellclass,umap.tx.clusters, umap.tx.cellcycle, ncol = 3))
dev.off()
